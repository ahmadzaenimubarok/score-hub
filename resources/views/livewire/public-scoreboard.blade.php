<div class="h-dvh w-screen flex flex-col overflow-hidden"
    ...
    <div class="flex-1 bg-[#4ade80] flex flex-col items-center justify-center min-h-0 p-1.5 relative">
        ...
        <div class="bg-[#1e1e2e] rounded-2xl shadow-2xl w-full h-full max-h-[80%]
            ...
            {{-- Map sides to left/right based on court flip --}}
            @php
                $leftName = $courtFlipped ? $match->name_b : $match->name_a;
                $rightName = $courtFlipped ? $match->name_a : $match->name_b;
                $leftScore = $courtFlipped ? ($scores[1] ?? 0) : ($scores[0] ?? 0);
                $rightScore = $courtFlipped ? ($scores[0] ?? 0) : ($scores[1] ?? 0);
                $leftSide = $courtFlipped ? 2 : 1;
                $rightSide = $courtFlipped ? 1 : 2;
                ...
                $clickable = !$matchOver && !$showSwitchCourt;
            @endphp
            ...
            <div class="flex-1 w-full flex flex-row items-stretch justify-center gap-3 sm:gap-8 py-2 sm:py-6">
                ...
                <div @if($clickable)
                     @mousedown.prevent="startLongPress({{ $leftSide }})"
                     @mouseup.prevent="clickTeam({{ $leftSide }}); endLongPress()"
                     @mouseleave.prevent="endLongPress()"
                     @touchstart.prevent="startLongPress({{ $leftSide }})"
                     @touchend.prevent="clickTeam({{ $leftSide }}); endLongPress()"
                     @endif
                ...
                {{-- RIGHT SIDE --}}
                <div @if($clickable)
                     @mousedown.prevent="startLongPress({{ $rightSide }})"
                     @mouseup.prevent="clickTeam({{ $rightSide }}); endLongPress()"
                     @mouseleave.prevent="endLongPress()"
                     @touchstart.prevent="startLongPress({{ $rightSide }})"
                     @touchend.prevent="clickTeam({{ $rightSide }}); endLongPress()"
                     @endif
                     class="flex-1 flex flex-col items-center justify-center
                            @if($clickable) cursor-pointer select-none @endif rounded-xl transition-colors duration-150
                            {{ $matchOver || !$clickable ? 'opacity-60 pointer-events-none' : 'hover:bg-white/5' }}
